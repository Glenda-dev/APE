...
impl<'a> ApeManager<'a> {
    ...
    fn setup_initial_tls(&mut self, pid: usize) -> Result<usize, Error> {
        let tls_start = glenda::mem::get_utcb_va(0)
    ...
    pub(crate) fn do_execve(
...

        {
            ...
            mm.heap_brk = heap_start;
            mm.heap_limit = mm.mmap_base;
            mm.mmap_next = mm.mmap_base;
            mm.stack_bottom = STACK_BASE;
            mm.stack_size = 0;
        }
        ...
        let (tcb_cap, fault_ep) = {
            let task = self.get_process(pid).ok_or(Error::NotFound)?;
            let fault_ep = Endpoint::from(CapPtr::concat(task.cspace.cap(), APE_SLOT));
            (task.tcb.clone(), fault_ep)
        };
